...
function MainContent() {
  ...
  const fetchPosts = async () => {
    ...
    try {
      const response = await axios.post(
        `https://universenode-chfnd3h6awepesd0.southindia-01.azurewebsites.net/userPost/getPostsForHome`,
        {
  ...
  const addSeenPost = useCallback(async (postId) => {
    ...
    try {
      await axios.post(
        "https://universenode-chfnd3h6awepesd0.southindia-01.azurewebsites.net/userPost/seenas",
        {
          userId: JSON.parse(localStorage.getItem("user")).id,
          postId: postId,
        },
